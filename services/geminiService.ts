...
const getAiClient = () => {
  // Try Vite env var first, fallback to process.env for AI Studio compatibility
  const apiKey = import.meta.env.VITE_GEMINI_API_KEY || process.env.GEMINI_API_KEY || "";
  if (!apiKey) {
    throw new Error("A chave da API do Gemini não está configurada. Por favor, adicione a variável de ambiente VITE_GEMINI_API_KEY no Netlify.");
  }
